...
var default_config_files = ['foreverize.json', 'foreverize.js'];
...
function load_config(config_file, options){
  ...
  if(!config_file){
    for(var i in config_file_locations){
      for(var j in default_config_files){
        var check_path = path.join(config_file_locations[i], default_config_files[j]);
